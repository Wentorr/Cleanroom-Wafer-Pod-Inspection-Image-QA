...
def split_group(scene_idx: int, rng: random.Random) -> tuple[str, str, str, str]:
    ...
    if scene_idx < 40:
        return "ood_occlusion", "occlusion", "normal", "high"
    return "ood_dense_clutter", "dense_clutter", rng.choice(["normal", "blue_cast"]), "high"
...
def draw_scene(trace: dict, out_path: Path, np_rng: np.random.Generator) -> None:
    ...
    for i, (port, c) in enumerate(trace["cable_map"].items()):
        ...
        line(base, x0, cable_origin_y, x1, y1, c, 4)

    robot_base = (360, 300)
    target = module_centers[trace["robot_target"]]
    circle(base, robot_base[0], robot_base[1], 18, "gray")
    line(base, robot_base[0], robot_base[1], target[0], target[1], "orange", 6)
    ...
    n_clutter = {"low": 18, "medium": 42, "high": 90}[clutter]
    ...
    if trace["lighting_condition"] == "low_light":
        ...
        for _ in range(3):
            gx = int(np_rng.integers(240, 600))
            gy = int(np_rng.integers(45, 280))
            blend_rect(base, gx, gy, gx + 110, gy + 36, "white", 0.46)

    if trace["ood_axis"] == "occlusion":
        blend_rect(base, 260, 132, 650, 164, "gray", 0.72)
        blend_rect(base, 110, 348, 620, 374, "black", 0.50)

    noise = np_rng.normal(0, 5.5, base.shape)
